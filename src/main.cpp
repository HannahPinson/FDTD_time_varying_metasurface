...
int main(){
	...
	int nodes_per_zone = 500 * grid_scaling;//1000 * grid_scaling; //! - nodes_per_zone: nodes per zone, used for changing the number of nodes without changing relative positions of materials and sources
	...
	double gamma = 0.09;
	double a = 0.6;
	double freq[3] = {0.5 * pow(10.0, 9), 1.00 * pow(10.0, 9), 1.5 *pow(10.0, 9)  };

	for (int i = 1; i < 2; i++){


		double initial_frequency =  freq[i]; //! - initial_frequency: frequency of the source signal, in Hz
		...
		//can be used for multiple sheets
		...
		try{ //try --> if error is 'thrown', rest of program skipped until 'catch' block catches the error
			...
			int readout_node1 = source_position  ;
			//int readout_node2 = 5012;
			sample_nodes.push_back(readout_node1);
			//sample_nodes_e.push_back(readout_node2);

			vector<int> no_sample_nodes_h;

			int snapshotmodulo_electric = -1;//500*grid_scaling;
			int snapshotmodulo_magnetic = -1;//500*grid_scaling;
			string output_path = "/Users/Hannah_Pinson/Desktop/FDTD_metasurface_proper/test_output/";
			...
			int timesteps = 400 * grid_scaling / S_c ;
